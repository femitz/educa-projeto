...
import { type BreadcrumbItem, type Categoria, type Curso } from '@/types';
...
interface AdminCursosEditProps {
    curso: Curso;
    categorias: Categoria[];
}

export default function AdminCursosEdit({ curso, categorias }: AdminCursosEditProps) {
    const cursoCategoriasIds = curso.categorias?.map((c) => c.id) || [];
    const [selectedCategorias, setSelectedCategorias] = React.useState<number[]>(cursoCategoriasIds);
    ...
    return (
        <AppLayout breadcrumbs={breadcrumbs}>
            <Head title={`Editar Curso - ${curso.nome}`} />
            <div className="flex h-full flex-1 flex-col gap-6 overflow-x-auto rounded-xl p-4">
                ...
                <Card>
                    ...
                    <CardContent>
                        <Form
                            action={`/admin/cursos/${curso.id}`}
                            method="patch"
                            data={{ categorias: selectedCategorias }}
                            className="space-y-6"
                        >
                            {({ processing, errors }) => (
                                <>
                                    ...
                                    <div className="grid grid-cols-2 gap-4">
                                        <div className="grid gap-2">
                                            ...
                                            <Input
                                                ...
                                                type="number"
                                                min="1"
                                                defaultValue={curso.tempo_horas}
                                                required
                                                placeholder="Ex: 40"
                                            />
